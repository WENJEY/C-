...
void editFullName(int idx) {
	...
	if (!isValidName(name)) {
		cout << " Invalid name. Use letters and spaces, 2-50 characters." << endl;
		return;
	}
	formatName(name);
...
void editAge(int idx) {
	...
	if (!isValidAge(age)) {
		cout << " Age must be 18 to 120." << endl;
		return;
	}
...
void editEmail(int idx) {
	...
	if (!isValidEmail(email)) {
		cout << " Invalid email. Use Gmail format: yourname@gmail.com" << endl;
		return;
	}
